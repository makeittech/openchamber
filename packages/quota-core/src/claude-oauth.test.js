...
describe('claude-oauth', () => {
  ...
  it('falls back to the last-known access token when refresh is unavailable', async () => {
    const access = await ensureClaudeUsageAccessToken({
      env: {},
      homeDir: '/missing-home',
      existsSync: () => false,
    ...
    expect(access).toMatchObject({ accessToken: 'no-refresh-access', canRefresh: false });
  });
});
