import { Buffer } from "node:buffer";
export const CURSOR_SELECTION_HEADER = "x-opencode-cursor-selection";
export function encodeCursorModelSelection(selection) {
    return Buffer.from(JSON.stringify(selection), "utf8").toString("base64url");
}
export function decodeCursorModelSelection(encoded) {
    if (!encoded || encoded.length > 8_192)
        return undefined;
    try {
        const value = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
        if (!value || typeof value !== "object" || Array.isArray(value))
            return undefined;
        const record = value;
        if (typeof record.publicId !== "string" ||
            !record.publicId.trim() ||
            typeof record.modelId !== "string" ||
            !record.modelId.trim() ||
            typeof record.displayName !== "string" ||
            typeof record.maxMode !== "boolean" ||
            !Array.isArray(record.parameters) ||
            record.parameters.length > 32) {
            return undefined;
        }
        const parameters = [];
        for (const parameter of record.parameters) {
            if (!parameter || typeof parameter !== "object" || Array.isArray(parameter)) {
                return undefined;
            }
            const item = parameter;
            if (typeof item.id !== "string" ||
                !item.id.trim() ||
                item.id.length > 128 ||
                typeof item.value !== "string" ||
                item.value.length > 256) {
                return undefined;
            }
            parameters.push({ id: item.id, value: item.value });
        }
        return {
            publicId: record.publicId,
            modelId: record.modelId,
            displayName: record.displayName,
            parameters,
            maxMode: record.maxMode,
        };
    }
    catch {
        return undefined;
    }
}
export function literalCursorModelSelection(modelId) {
    return {
        publicId: modelId,
        modelId,
        displayName: modelId,
        parameters: [],
        maxMode: false,
    };
}
